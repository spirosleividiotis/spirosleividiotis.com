...
function updateHeader(header) {
    ...
    if (leftEl) {
        ...
        leftEl.innerHTML = leftItems.map((item, i) => {
            ...
            return `<a href="${escapeAttr(href)}" class="hero-link" ${isAbout ? 'id="aboutMeLink"' : ''}>${escapeHtml(label)} ${heroArrowSvg}</a>`;
        }).join('');
    }
    if (centerEl && nav.length >= 3) {
        ...
        centerEl.innerHTML = `<a href="${escapeAttr(href)}" class="hero-link">${escapeHtml(label)} ${heroArrowSvg}</a>`;
    } else if (centerEl && nav.length === 2) {
        centerEl.innerHTML = `<a href="#work" class="hero-link">projects ${heroArrowSvg}</a>`;
    } else if (centerEl) {
        centerEl.innerHTML = '';
    }
}
